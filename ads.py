...
def klik_tombol_pertama(page):
    try:
        ...
        iframe1.locator("button.gpxLoader-button").click(timeout=8000)
        return True
    except:
        return False
...
def cari_dan_klik_learn_more(page):
    ...
    teks_tombol_list = [
        ...
        "sign up", "website", "selengkapnya", "daftar", "buka", "shop now"
    ]

    # --- TRIK BARU: SCROLL BIAR IKLAN KEPANCING KELUAR ---
    ...
    for percobaan in range(5):
            ...
            for teks in teks_tombol_list:
                try:
                    ...
                    if tombol.is_visible():
                        # Simulasi gerakan manusia sebelum klik
                        tombol.hover()
                        time.sleep(1)
        ...
        if found: break
        time.sleep(3) # Tunggu iklan beneran ke-load kalau belum ketemu
...
def run_simulasi(sesi_ke):
    ...
    with sync_playwright() as p:
        ...
        # URL BLOG LU
        url = "https://gamesfreeonlinehub.blogspot.com/2026/01/prism-match-3d.html?m=1"
        ...
        try:
            ...
            # 1. Klik Tombol Play Awal
            if not klik_tombol_pertama(page):
                print(f"⚠️ Sesi ke-{sesi_ke}: Tombol Play gak ketemu, lanjut cari iklan.")

            # 2. Kasih waktu iklan loading (PENTING: Durasi ditambah)
            tunggu_iklan = random.randint(18, 25)
            print(f"⏳ Menunggu iklan loading ({tunggu_iklan} detik)...")
            time.sleep(tunggu_iklan)

            # 3. Eksekusi Klik Iklan
            learn_more_diklik = cari_dan_klik_learn_more(page)

            if learn_more_diklik:
                # REVENUE BOOSTER: Ngetem lama di halaman iklan biar Dollar valid
                waktu_tunggu = random.randint(40, 55)
                print(f"🔥 IKLAN KETEMU! Ngetem {waktu_tunggu} detik biar Revenue cair...")
            else:
            ...
            print(f"❌ Kesalahan pada sesi ke-{sesi_ke}: {e}")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            sesi_ke += 1
            # Kasih jeda antar sesi biar gak dianggap ddos
